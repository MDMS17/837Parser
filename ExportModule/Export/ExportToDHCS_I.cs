        ...
        public static void Export837I(string CountyCode)
        {
            ...
            for (int i = 0; i <= pages; i++)
            {
                ...
                if (headers.Count > 0)
                {
                    ...
                    File.WriteAllText(Path.Combine(DestinationFolder, "IEHP_" + CountyCode + "_837I_MCE_" + DateTime.Today.ToString("yyyyMMdd") + "_" + (i + 1).ToString().PadLeft(5, '1') + ".dat"), sb.ToString());
                }
            }
            context.Dispose();
        }
